...
class Storage:
    def __init__(self):
        ...
        self.active_orders: Dict[
            uuid.UUID, Tuple[str, OrderDeliveryStatus]
        ] = {}
...
class Scheduler:
    ...
    def ship_order(self, order_name: str) -> None:
        ...
        instance = ConcreteDeliveryService(
            order=DeliveryOrder(order_name=order_name)
        )
        instance.ship()
        ...
        while True:
            ...
            with storage.lock:
                ...
                to_archive = [
                    ...
                    if (
                        now - end_time
                    ) >= timedelta(seconds=ARCHIVE_CHECK_INTERVAL)
                ]
...
def main():
    ...
    order_thread = threading.Thread(
        target=scheduler.process_orders,
        daemon=True
    )
    order_thread.start()
    ...
    archiver_thread = threading.Thread(
        target=scheduler.check_archived_orders,
        daemon=True
    )
